...
/*    		var guidanceItem = guidanceItems.tmGuidanceItem(expectedId.guid()); 
			...
			Assert.That(guidanceItem.Id.str() == expectedId,"expected Id didn't match: {0} it was '{1}'".format(expectedId, guidanceItems[0].Metadata.Id));
    		Assert.That(guidanceItem.Title == expectedTitle,"expected Title didn't match: {0} it was '{1}'".format(expectedTitle, guidanceItems[0].Metadata.Title));
*/    	
    		//Assert.That(guidanceItem.Topic == expectedTopic,"expected Topic didn't match: {0} it was '{1}'".format(expectedTopic, guidanceItems[0].Metadata.Topic));    		
    	...
    	{    
    		...
    		var html = tmWebServices.javascriptProxy.GetGuidanceItemHtml(guidanceItem.Metadata.Id);    		
